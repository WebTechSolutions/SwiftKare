    ...
    public class DoctorAppointmentController : Controller
    {
        ...
        public PartialViewResult ViewAppDetails(long? appID)
        {
            try
            {
                ...
                if(oData!=null)
                {
                    if (oData.PatientVM.patPicture != null && oData.PatientVM.patPicture.Count() > 0)
                    {
                        oData.PatientVM.ProfilePhotoBase64 = System.Text.Encoding.ASCII.GetString(oData.PatientVM.patPicture);
                    }
                    else
                    {
