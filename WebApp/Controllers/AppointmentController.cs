...
namespace WebApp.Controllers
{
    ...
    public class AppointmentController : Controller
    {
        ...
        public JsonResult UpdatePatientTimezone(string offset)
        {
            try
            {
                ApiResultModel apiresult = new ApiResultModel();
        ...
        public JsonResult GetTimeZones()
        {
            ...
            {
                ProfileRepository oProfileRepository = new ProfileRepository();
                var timezones = oProfileRepository.GetTimeZones();

                return Json(new { Success = true, Object = timezones });
        ...
        public PartialViewResult DoctorProfile(long doctorID)
        {
            try
            {
                ...
                oModel.fav = 0;
                Repositories.DoctorRepositories.SeeDoctorRepository objDoctorRepo = new Repositories.DoctorRepositories.SeeDoctorRepository();
                var favdoc = objDoctorRepo.LoadFavDoctors(SessionHandler.UserInfo.Id);
                
                if (oModel != null)
                {
                    foreach(var item in favdoc)
                    {
                        if(item.docID==doctorID)
                        {
                            oModel.fav = 1;
                        }
                    }
