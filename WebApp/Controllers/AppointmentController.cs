    ...
    public class AppointmentController : Controller
    {
        ...
        public PartialViewResult ViewAppDetails(long? appID)
        {
            try
            {
                ...
                if (oData != null)
                {
                    if (oData.DoctorVM.docPicture != null && oData.DoctorVM.docPicture.Count() > 0)
                    {
                        oData.DoctorVM.ProfilePhotoBase64 = System.Text.Encoding.ASCII.GetString(oData.DoctorVM.docPicture);
                    }
                    else
                    {
